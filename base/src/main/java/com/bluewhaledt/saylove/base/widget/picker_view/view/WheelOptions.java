package com.bluewhaledt.saylove.base.widget.picker_view.view;

import android.view.View;

import com.bluewhaledt.saylove.base.R;
import com.bluewhaledt.saylove.base.widget.picker_view.adapter.ArrayWheelAdapter;
import com.bluewhaledt.saylove.base.widget.picker_view.lib.WheelView;
import com.bluewhaledt.saylove.base.widget.picker_view.listener.OnItemSelectedListener;
...
public class WheelOptions<T> {
	...
	public void setPicker(ArrayList<T> options1Items,
		...
		wv_option1.setAdapter(new ArrayWheelAdapter<T>(mOptions1Items, len));// 设置显示数据
		wv_option1.setCurrentItem(0);// 初始化时显示的数据
		// 选项2
		wv_option2 = (WheelView) view.findViewById(R.id.options2);
		if (mOptions2Items != null)
			wv_option2.setAdapter(new ArrayWheelAdapter<T>(mOptions2Items.get(0)));// 设置显示数据
		wv_option2.setCurrentItem(wv_option1.getCurrentItem());// 初始化时显示的数据

		// 选项3
		wv_option3 = (WheelView) view.findViewById(R.id.options3);
		if (mOptions3Items != null){
			wv_option3.setAdapter(new ArrayWheelAdapter<T>(mOptions3Items.get(0).get(0)));// 设置显示数据
			if (mOptions3Items.get(0).get(0).size() > 0){
...
//                    opt2Select = opt2Select >= mOptions2Items.get(index).size() - 1 ? mOptions2Items.get(index).size() - 1 : opt2Select;
					wv_option2.setAdapter(new ArrayWheelAdapter<T>(mOptions2Items
							.get(index)));
...
//                    opt3 = opt3 >= mOptions3Items.get(opt1Select).get(index).size() - 1 ? mOptions3Items.get(opt1Select).get(index).size() - 1 : opt3;
					...
					if (mOptions3Items.get(wv_option1.getCurrentItem()).size() > 0 && mOptions3Items.get(wv_option1.getCurrentItem()).get(index).size() > 0){
						wv_option3.setVisibility(View.VISIBLE);
						wv_option3.setAdapter(new ArrayWheelAdapter<T>(mOptions3Items.get(wv_option1.getCurrentItem()).get(index)));
...
//		// 添加联动监听
	...
	public ArrayList<T> getCurrentItemsModel(){
		ArrayList<T> data = new ArrayList<>();
		if (mOptions1Items != null && mOptions1Items.size() > 0){
			data.add(mOptions1Items.get(wv_option1.getCurrentItem()));
		}else{
			data.add(null);
		}
		if (mOptions2Items != null && mOptions2Items.size() > 0){
			data.add(mOptions2Items.get(wv_option1.getCurrentItem()).get(wv_option2.getCurrentItem()));
		}else{
			data.add(null);
		}

		if (mOptions3Items != null && mOptions3Items.size() > 0){
			if (mOptions3Items.get(wv_option1.getCurrentItem()).size() > 0){
				ArrayList<T> item = mOptions3Items.get(wv_option1.getCurrentItem()).get(wv_option2.getCurrentItem());
				if (item != null && item.size() > 0 ){
					data.add(item.get(wv_option3.getCurrentItem()));
				}else{
					data.add(null);
				}
			}
	...
	private void itemSelected(int opt1Select, int opt2Select, int opt3Select) {
		...
		if (mOptions3Items != null && mOptions3Items
				.get(opt1Select).size() > 0) {
