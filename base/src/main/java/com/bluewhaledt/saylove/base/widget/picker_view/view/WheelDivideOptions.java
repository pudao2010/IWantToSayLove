...
public class WheelDivideOptions<T> {
    private View view;
    ...
    public WheelDivideOptions(View view) {
        super();
        this.view = view;
        setView(view);
    }
...
//        this.linkage = linkage;
        ...
        if (mOptions2Items != null)
            wv_option2.setAdapter(new ArrayWheelAdapter<T>(mOptions2Items));// 设置显示数据
        wv_option2.setCurrentItem(wv_option1.getCurrentItem());// 初始化时显示的数据
        // 选项3
        wv_option3 = (WheelView) view.findViewById(R.id.options3);
        if (mOptions3Items != null)
            wv_option3.setAdapter(new ArrayWheelAdapter<T>(mOptions3Items));// 设置显示数据
        ...
        wheelListener_option2 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                if (mOptions3Items != null) {
                    ...
                    wv_option3.setAdapter(new ArrayWheelAdapter<T>(mOptions3Items));
...
//		// 添加联动监听
    ...
    public ArrayList<T> getCurrentItemsModel(){
        ...
        if (mOptions2Items != null && mOptions2Items.size() > 0){
            data.add(mOptions2Items.get(wv_option2.getCurrentItem()));
        }else{
            data.add(null);
        }

        if (mOptions3Items != null && mOptions3Items.size() > 0){
            data.add(mOptions3Items.get(wv_option3.getCurrentItem()));
        }else{
            data.add(null);
        }
