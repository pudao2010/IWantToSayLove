package com.bluewhaledt.saylove.widget.crop;
...
public class CropImage {
    ...
    public void startRotate(float d) {
        ...
        showProgressDialog(
                mContext.getResources().getString(R.string.txt_wait),
                new Runnable() {
                    public void run() {
    ...
    private Bitmap onSaveClicked(Bitmap bm) {
        ...
        {
            System.out.println("原始bitma>>" + bm.getWidth() + "---"
                    + bm.getHeight());
            Canvas canvas = new Canvas(croppedImage);
            Rect dstRect = new Rect(0, 0, width, height);
            canvas.drawBitmap(bm, r, dstRect, null);
    ...
    public static String saveToLocal(Bitmap bm, Context context) {
        if (bm == null)
            return "";
        String path = new File(FileUtils.getCacheDirectory(context),
                String.valueOf(System.currentTimeMillis())).toString() + Constants.SAVE_PIC_FORMAT;
        try {
    ...
    public static String saveToLocal(Bitmap bm) {
        if (bm == null)
            return "";
        String path = new File(FILE_SDCARD,
...
//         msg);
    ...
    Runnable mRunFaceDetection = new Runnable() {
        ...
        private void makeDefault() {
            ...
            // zhx modify: 原处理方法，宽高一致
            int cropWidth = Math.min(width, height) * 4 / 5;
            int cropHeight = cropWidth;
            // zhx add:新处理方法，宽高比4:5的处理方法
            // 取最小边作为高，高的4/5就是宽
            // int cropHeight = Math.min(width, height) * 4 / 5;
            // int cropWidth = cropHeight * 4 / 5;
    ...
    class BackgroundJob implements Runnable {
        ...
        public void run() {
            ...
            mHandler.post(new Runnable() {
                public void run() {
                    try {
                        ...
                        // .obtainMessage(AvatarUploadActivity.SHOW_PROGRESS));
            ...
            try {
                ...
                // mHandler.sendMessage(mHandler
                // .obtainMessage(AvatarUploadActivity.REMOVE_PROGRESS));
            }
        }
    }
}
