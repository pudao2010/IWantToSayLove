...
import com.bluewhaledt.saylove.video_record.listener.CommonHandlerListener;
...
public class CameraController
    ...
    private CameraController() {
        ...
        sCameraRatio = (ZhenaiApplication.getInstance().getScreenHeight()-statusBarHeight )* 1.0f /
                ZhenaiApplication.getInstance().getScreenWidth();
...
//                LocalBroadcastManager.getInstance(context).sendBroadcast(intent);
                EventBus.getDefault().post(new CameraOpenFailEvent(true));
            }
            ...
            try {
                ...
                EventBus.getDefault().post(new CameraOpenFailEvent(false));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private void findCameraSupportValue(int desiredWidth) {
        ...
        if (cs != null && !cs.isEmpty()) {
            ...
            for (Camera.Size size : cs) {
                ...
                float currentDiff = Math.abs(sCameraRatio - ratio);
                if (currentDiff < diff) {           //修正人脸拉长问题  找最接近的比例
                    diff = currentDiff;
...
//                }
    ...
    //PictureCallback
    //@Override public void onPictureTaken(byte[] intentData, Camera camera) {
    //    mIsTakingPicture = false;
    //    //try {
    //    //    Camera.Parameters ps = camera.getParameters();
    //    //    if (ps.getPictureFormat() == ImageFormat.JPEG) {
    //    //        //CommonUtil.executeAsyncTask(new SquareBitmapTask(intentData, mCameraMirrored) {
