...
import com.bluewhaledt.saylove.video_record.camera.CameraPreviewSizeComparator;
...
public class SystemAvRecorder extends BaseAvRecorder implements SurfaceHolder.Callback {
    ...
    public SystemAvRecorder(SurfaceView surfaceView) {
    ...
    @Override
    public void stopRecord(boolean shouldCallback) {
        // 如果正在录制，停止并释放资源
        if (mMediaRecorder != null) {
            mMediaRecorder.stop();
            mMediaRecorder.release();
    ...
    private void findBestPreviewSize() {
        ...
        float targetRatio = (ZhenaiApplication.getInstance().getScreenHeight() - statusBarHeight) * 1.0f / ZhenaiApplication.getInstance().getScreenWidth();
        ;
        ...
        mCamera.getParameters().setPreviewSize(optimalSize.height, optimalSize.width);
    }
}
