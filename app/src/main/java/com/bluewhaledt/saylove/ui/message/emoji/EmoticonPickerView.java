...
public class EmoticonPickerView extends LinearLayout implements IEmoticonCategoryChanged {
    ...
    public void setListener(IEmoticonSelectedListener listener) {
        if (listener != null) {
            ...
            DebugUtils.i("sticker", "listener is null");
        }
    }
    ...
    private CheckedImageButton addEmoticonTabBtn(int index, OnClickListener listener) {
        ...
        emotBtn.setPaddingValue(DensityUtils.dp2px(context,7));

        final int emojiBtnWidth = DensityUtils.dp2px(context,50);
        final int emojiBtnHeight = DensityUtils.dp2px(context,44);
    ...
    private void show() {
        if (listener == null) {
            DebugUtils.i("sticker", "show picker view when listener is null");
        }
        if (!withSticker) {
            showEmojiView();
        } else {
