...
public class MyHeartBeatRecordModel implements IBaseMode<HeartBeatItem> {
    @Override
    public void getDataList(Context context, int pageIndex, int pageSize, BaseSubscriber<ZAResponse<ResultEntity<HeartBeatItem>>> subscriber) {
        ...
        Observable<ZAResponse<ResultEntity<HeartBeatItem>>> observable = service.getMyHeartBeatList(pageIndex,pageSize);
